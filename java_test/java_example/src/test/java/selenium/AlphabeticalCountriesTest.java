...
public class AlphabeticalCountriesTest {
    ...
    //функция для проверки алфавитного порядка
    ...
    public void alphabeticalCountriesTest() throws InterruptedException {
        ...
        if (driver.findElement(By.id("content")).isDisplayed()) {
            ...
            for (WebElement country : countries) {
                //проверка названий стран на соответствие алфавитному порядку
                ...
                //если количество зон больше нуля, то проверяем алфавитный порядок зон
                if (zonesCount != 0) {
                    WebElement button = country.findElement(By.xpath("//a[contains (@title, 'Edit')]"));
                    button.click(); // зашли на страницу со списком зон

                    wait.until(ExpectedConditions.visibilityOfElementLocated(By.className("dataTable")));
                    if (driver.findElement(By.id("content")).isDisplayed()) {
                        var zones =
                                driver.findElements(By.xpath("//table[@class='dataTable']//td[3]/input[contains(@name, 'zones')]"));
                        ...
                        driver.navigate().back(); // вернулись на страницу со списком стран
                    }
                }
            }
        }
